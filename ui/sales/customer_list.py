...
class CustomerDialog(QDialog):
    def __init__(self, parent=None, customer=None):
        ...
        self.name_edit.setToolTip("Customer's full name (required)")
        self.phone_edit = QLineEdit()
        self.phone_edit.setToolTip("Contact phone number")
        self.address_edit = QLineEdit()
        self.address_edit.setToolTip("Customer's billing address")
        self.gstin_edit = QLineEdit()
        self.gstin_edit.setToolTip("GST registration number (if business customer)")
        self.credit_limit = QDoubleSpinBox()
        self.credit_limit.setRange(0, 999999)
        self.credit_limit.setDecimals(2)
        self.credit_limit.setToolTip("Maximum credit allowed for this customer")
        ...
        save_btn.setToolTip("Save this customer and return")
...
class CustomerListWidget(QWidget):
    def __init__(self):
        ...
        add_btn.setToolTip("Register a new customer")
        add_btn.clicked.connect(self._add)
        header.addWidget(add_btn)
        layout.addLayout(header)

        subtitle = QLabel("Manage customer accounts, credit limits, and balances")
    ...
    def refresh(self):
        ...
        for i, c in enumerate(customers):
            ...
            edit_btn.setToolTip("Edit this customer's details")
            edit_btn.setFixedWidth(60)
            edit_btn.clicked.connect(lambda checked, cid=c["id"]: self._edit(cid))
            del_btn = QPushButton("Del")
            del_btn.setObjectName("dangerBtn")
            del_btn.setToolTip("Delete this customer permanently")
